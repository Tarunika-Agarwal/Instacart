...
function Pizzas() {
  ...
  const restaurantNames = PizzaData.map(item => item.rname);

  const handleRestaurantSelect = (event, value) => {
    if (value) {
      const filteredData = PizzaData.filter(item => item.rname === value);
      setData(filteredData);
    } else {
      setData(PizzaData); // Reset to default data if no selection
    }
