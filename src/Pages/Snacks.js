...
import SnackData from '../components/SnackData';
...
function Snacks() {
  ...
  const restaurantNames = SnackData.map(item => item.rname);

  const handleRestaurantSelect = (event, value) => {
    if (value) {
      const filteredData = SnackData.filter(item => item.rname === value);
      setData(filteredData);
    } else {
      setData(SnackData); // Reset to default data if no selection
    }
